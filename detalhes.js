...
async function iniciar() {
  ...
  try {
    const [resposta, respostaMateriais] = await Promise.all([fetch("data/concursos.json"), fetch("data/materiais.json")]);
    ...
    $("#detail-heading").innerHTML = `<p class="eyebrow">${seguro(nomesStatus[c.status])}</p><h1>${seguro(c.orgao)}</h1><p class="hero-copy">${seguro(c.titulo)}</p><p class="verified">Última verificação: ${data(c.ultimaVerificacao)} · ${seguro(c.tipoFonte)}</p>`;
    const salario = c.salarioMinimo || c.salarioMaximo ? `${dinheiro(c.salarioMinimo)} a ${dinheiro(c.salarioMaximo)}` : "Não informado";
    const recomendados = materiais.filter((m) => m.tags.includes("todos") || m.tags.some((tag) => (c.tags || []).map((t) => t.toLowerCase()).includes(tag.toLowerCase()))).slice(0, 4);
    $("#detail-content").innerHTML = `<div class="detail-main"><section class="detail-card"><h2>Visão geral</h2><p>${seguro(c.descricao)}</p><div class="facts"><div><span>Situação</span><strong>${seguro(nomesStatus[c.status])}</strong></div><div><span>Local</span><strong>${seguro(c.cidade)} — ${seguro(c.estado)}</strong></div><div><span>Vagas</span><strong>${seguro(c.vagas)}</strong></div><div><span>Cadastro reserva</span><strong>${seguro(c.cadastroReserva)}</strong></div><div><span>Escolaridade</span><strong>${seguro(c.escolaridade.join(", "))}</strong></div><div><span>Salário</span><strong>${salario}</strong></div><div><span>Banca</span><strong>${seguro(c.banca)}</strong></div><div><span>Taxa</span><strong>${seguro(c.taxa)}</strong></div></div></section><section class="detail-card"><h2>Linha do tempo</h2><ol class="timeline">${c.linhaDoTempo.map((item) => `<li class="${item.concluida ? "done" : ""}"><span></span><strong>${seguro(item.etapa)}</strong></li>`).join("")}</ol></section><section class="detail-card"><h2>Cargos</h2><ul>${c.cargos.map((cargo) => `<li>${seguro(cargo)}</li>`).join("")}</ul></section><section class="detail-card"><h2>Atualizações</h2>${c.atualizacoes.length ? `<ul>${c.atualizacoes.map((a) => `<li>${seguro(a.titulo || a)} ${a.data ? `— ${data(a.data)}` : ""}</li>`).join("")}</ul>` : '<p class="muted">Nenhuma atualização adicional cadastrada.</p>'}</section><section class="detail-card"><h2>Materiais para sua preparação</h2><p class="muted">Sugestões do Prof. Lucas MPC relacionadas à organização e à preparação para concursos.</p><div class="materials-grid">${recomendados.map((m) => `<a class="material-card" href="${m.url}" target="_blank" rel="noopener"><span>${seguro(m.tipo)}</span><strong>${seguro(m.titulo)}</strong><p>${seguro(m.descricao)}</p></a>`).join("")}</div></section></div><aside class="detail-side"><section class="detail-card"><h2>Datas importantes</h2><dl><dt>Início das inscrições</dt><dd>${data(c.inicioInscricoes)}</dd><dt>Fim das inscrições</dt><dd>${data(c.fimInscricoes)}</dd><dt>Data da prova</dt><dd>${data(c.dataProva)}</dd></dl></section><section class="detail-card action-stack"><a class="primary-action" href="${c.editalUrl}" target="_blank" rel="noopener">Acessar edital oficial</a><a class="secondary-action" href="${c.inscricaoUrl}" target="_blank" rel="noopener">Página de inscrição</a><a class="secondary-action" href="${c.fonteUrl}" target="_blank" rel="noopener">Ver fonte oficial</a></section><section class="source-note"><strong>Aviso importante</strong><p>Confirme todos os dados na fonte oficial antes de realizar sua inscrição.</p></section></aside>`;
