...
function dataLocal(valor) { if (!valor) return null; const [ano, mes, dia] = valor.split("-").map(Number); return new Date(ano, mes - 1, dia); }
function hojeLocal() { const agora = new Date(); return new Date(agora.getFullYear(), agora.getMonth(), agora.getDate()); }
function statusEfetivo(c) {
  if (!["aberto", "edital"].includes(c.status) || !c.inicioInscricoes || !c.fimInscricoes) return c.status;
  const hoje = hojeLocal(), inicio = dataLocal(c.inicioInscricoes), fim = dataLocal(c.fimInscricoes);
  if (hoje < inicio) return "edital";
  if (hoje > fim) return "encerrado";
  return "aberto";
}
function diasAte(valor) { const fim = dataLocal(valor); return fim ? Math.ceil((fim - hojeLocal()) / 86400000) : null; }
...
function filtrarConcursos() {
  const busca = normalizar($("#busca").value.trim()), nivel = $("#nivel").value, estado = $("#estado").value, esfera = $("#esfera").value, banca = $("#banca").value, ordenacao = $("#ordenacao").value;
...
function render() {
  ...
  cards.innerHTML = lista.map(c => { const status=statusEfetivo(c), horario=c.horarioFimInscricoes?` às ${c.horarioFimInscricoes}`:""; return `<article class="card">${c.destaque?'<span class="badge">DESTAQUE</span>':""}<span class="tag status-${status}">${statusNomes[status]}</span>${etiquetaPrazo(c,status)}<h3>${c.orgao}</h3><p class="card-title">${c.titulo}</p><div class="meta"><span>📍 ${c.cidade} — ${c.estado}</span><span>👥 ${c.vagas}</span><span>🎓 ${escolaridadeTexto(c.escolaridade)}</span><span>🏛️ ${c.esfera[0].toUpperCase()+c.esfera.slice(1)}</span></div><div class="verification">Verificado em ${formatarData(c.ultimaVerificacao)}</div><div class="card-bottom"><div><div class="deadline">Prazo: ${formatarData(c.fimInscricoes)}${horario}</div><small>Banca: ${c.banca}</small></div><a class="details" data-lead-context="${c.orgao} — ${c.titulo}" href="detalhes.html?concurso=${encodeURIComponent(c.slug)}">Ver detalhes</a></div></article>`; }).join("");
  $("#empty").hidden = lista.length > 0;
}
function mostrarDados(){ if(liberado())return; captura.hidden=false; $("#etapa-dados").hidden=false; $("#etapa-codigo").hidden=true; erro.hidden=true; }
function mostrarCodigo(){ if(liberado())return; captura.hidden=false; $("#etapa-dados").hidden=true; $("#etapa-codigo").hidden=false; erro.hidden=true; setTimeout(()=>$("#codigo").focus(),50); }
function contextoDoClique(acao){ if(acao?.dataset?.leadContext)return acao.dataset.leadContext; const card=acao?.closest?.(".card"); if(card){const orgao=card.querySelector("h3")?.textContent?.trim(),titulo=card.querySelector(".card-title")?.textContent?.trim(); return [orgao,titulo].filter(Boolean).join(" — ");} return acao?.textContent?.trim()||"Radar de Concursos MPC"; }
function pedirCodigo(){ erro.hidden=true; const nome=$("#nome").value.trim(),objetivo=$("#objetivo").value; if(nome.length<2){erro.textContent="Informe seu nome para continuar.";erro.hidden=false;return;} localStorage.setItem("radarMpcCodigoPendente","true"); const interesse=localStorage.getItem("radarMpcUltimoInteresse")||"Radar de Concursos MPC"; mostrarCodigo(); const retorno=location.href||SITE_URL; const mensagem=encodeURIComponent(`Olá, Prof. Lucas! Meu nome é ${nome}. Quero meu código de acesso ao Radar de Concursos MPC. Meu objetivo: ${objetivo}.\n\nConcurso/interesse: ${interesse}.\n\nLink para eu retornar depois do código:\n${retorno}`); window.open(`https://api.whatsapp.com/send/?phone=${WHATSAPP_NUMBER}&text=${mensagem}`,"_blank","noopener"); }
...
async function iniciar(){
  try{
    ...
    const mapa=new Map(); lotes.flat().forEach(c=>mapa.set(c.id,c)); concursos=[...mapa.values()];
    preencherEstados();render();
  }catch{cards.innerHTML='<div class="empty"><h3>Não foi possível carregar as oportunidades</h3><p>Tente atualizar a página em alguns instantes.</p></div>';}
}
document.querySelectorAll(".tab[data-status]").forEach(tab=>tab.addEventListener("click",()=>{document.querySelector(".tab.active").classList.remove("active");tab.classList.add("active");statusAtual=tab.dataset.status;render();}));
["#busca","#nivel","#estado","#esfera","#banca","#ordenacao"].forEach(id=>$(id).addEventListener(id==="#busca"?"input":"change",render));
$("#buscar").onclick=()=>$("#oportunidades").scrollIntoView(); $("#fechar").onclick=()=>captura.hidden=true; $("#pedir-codigo").onclick=pedirCodigo; $("#liberar").onclick=validar; $("#codigo").oninput=e=>e.target.value=e.target.value.toUpperCase().replace(/[^A-Z0-9]/g,""); $("#codigo").addEventListener("keydown",e=>{if(e.key==="Enter")validar();});
document.addEventListener("click",evento=>{if(liberado()||!captura.hidden||evento.target.closest("#captura"))return;const acao=evento.target.closest("a, button");if(!acao)return;localStorage.setItem("radarMpcUltimoInteresse",contextoDoClique(acao));const href=acao.tagName==="A"?acao.getAttribute("href"):null;if(href&&!href.startsWith("#")&&!href.startsWith("javascript:"))evento.preventDefault();mostrarDados();},true);
if(!liberado()){if(localStorage.getItem("radarMpcCodigoPendente")==="true")mostrarCodigo();else setTimeout(()=>{if(!liberado()&&captura.hidden)mostrarDados();},60000);} iniciar();
